...
| [OUTPUT_AUDIO.md](OUTPUT_AUDIO.md) | MP3 / local TTS | Developers |
...
See [`.cursorrules`](../../.cursorrules) and [tests/AGENTS.md](../tests/AGENTS.md). Measure with `uv run pytest --collect-only -q` rather than hardcoding counts in prose.
...
| [OUTPUT_AUDIO.md](OUTPUT_AUDIO.md) | MP3 / local TTS output |
...
- Real library/tool calls by default (WeasyPrint, local TTS, ffmpeg, etc.)
- Real validation logic
- Temporary directories for isolation
- Limited test doubles are allowed for external-service boundaries and expensive orchestration seams when the test documents the boundary

### Test Markers

| Marker | Purpose |
|--------|---------|
| `requires_internet` | Tests requiring network |
| `requires_api` | Tests requiring external API |
| `audio` | Tests invoking local audio/TTS tooling |
...
| Property | Value |
|----------|-------|
| **Generator** | `text_to_speech` module (local `say` + `ffmpeg` on macOS) |
| **Extension** | `.mp3` |
| **Dependencies** | local TTS command and `ffmpeg` |
| **Known Limitations** | Slower than text/PDF outputs; command timeouts are enforced |
...
| Publish copy | `publish` | `publish_course()` |
| Question renumbering | `content_processing` | `renumber_questions_in_course()` |
| Legacy imports | `legacy_import` | `import_legacy_course()` |
...
| **MP3** | Local TTS/ffmpeg latency | Set `mp3 = false` in `publish.toml` for faster runs |
...
| Date | Changes |
|------|---------|
| 2026-05-04 | Aligned documentation standards text with `.cursorrules`/tests docs; dropped broken external report link |
