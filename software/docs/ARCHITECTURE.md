...
# 1. Narrow formats (omit mp3) for faster iteration — third arg is optional
process_module_by_type(
    module_path,
    output_dir,
...
**Published layout:** `index.html` is generated under each module’s `output/website/` in **`course_development/`**. The publish step **`reorganize_to_categories()`** deletes stray `index.html` files while building category folders in **`PUBLISHED/`**, so the public course repos do not carry the full interactive bundle unless the pipeline is changed.
