...
2. DNA methylation that silences a gene is often described with the memory trick “Methylation = Muting” because methylation tends to:
    ...
    - B) Turn the gene off or reduce its activity
    - C) Delete the gene from the chromosome
    - D) Replace thymine with uracil
...
1. **Information Flow and Regulation.** Trace the path from DNA to a functional protein (name the major steps). Then add one sentence explaining how epigenetics or gene regulation can change whether that gene’s product is actually made in a given cell type—without changing the DNA sequence.

2. **Mitosis vs. Meiosis.** Compare mitosis and meiosis in terms of: number of divisions, number of daughter cells, ploidy (haploid vs. diploid) of the products, and whether the daughter cells are genetically identical to each other. Also state one main purpose of each process.

3. **Inheritance in Words.** Explain the difference between genotype and phenotype. Then describe what incomplete dominance looks like in offspring compared with simple dominant/recessive inheritance (you may use a flower-color example).

4. **Epigenetics vs. Mutation.** Define epigenetics in your own words and contrast it with a DNA mutation. Give one example of an environmental factor that can influence gene activity through epigenetic-style regulation (diet, stress, exercise, or toxins—pick one and explain briefly).

5. **Biotechnology Toolkit.** Briefly explain what PCR accomplishes and why gel electrophoresis is useful afterward. OR explain how inserting a foreign gene into a bacterial plasmid relates to the universal genetic code (either option is acceptable—state which you chose).
